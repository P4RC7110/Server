...
ACMD_FUNC(rates)
{
	...
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1299), // Normal Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common+item_rate)/100., (battle_config.item_rate_heal)/100., (battle_config.item_rate_use)/100., (battle_config.item_rate_equip)/100., (battle_config.item_rate_card)/100.);
	clif_displaymessage(fd, buf);
...
if( level >= pc_get_group_level(sd) )
{
...
ACMD_FUNC(whosell)
{
	...
	for( i = 0; i < count; i++ )
	{
		...
		{
			sprintf(output, "[%d] The best price of '%s' is %u about '%s' in %s <%d,%d>. Max %u. Found in %d shops, %d on sale.", item_array[i]->nameid, item_array[i]->jname, MinPrice[i], b_sd[i]->status.name, map[b_sd[i]->bl.m].name, b_sd[i]->bl.x, b_sd[i]->bl.y, MaxPrice[i], total[i], amount[i]);
			if( sd->bl.m == b_sd[i]->bl.m )
				clif_viewpoint(sd, 1, 1, b_sd[i]->bl.x, b_sd[i]->bl.y, i, 0xFFFFFF);
		}
		else
			sprintf(output, "[%d] '%s' is not on stores...", item_array[i]->nameid, item_array[i]->jname);
